...
public:
    ...
    void NewBlock();
    CBudgetProposal* FindProposal(const std::string& sRIZroposalName);
    CBudgetProposal* FindProposal(uint256 nHash);
    CFinalizedBudget* FindFinalizedBudget(uint256 nHash);
    std::pair<std::string, std::string> GetVotes(std::string sRIZroposalName);
...
public:
    ...
    void AutoCheck();
    //total riz paid out by this budget
...
public:
    ...
    */
    ...
    CBudgetProposal(std::string sRIZroposalNameIn, std::string strURLIn, int nBlockStartIn, int nBlockEndIn, CScript addressIn, CAmount nAmountIn, uint256 nFeeTXHashIn);
    ...
    std::string GetName() { return sRIZroposalName; }
    ...
    {
        //for syncing with other clients
        READWRITE(LIMITED_STRING(sRIZroposalName, 20));
...
public:
    ...
    CBudgetProposalBroadcast(std::string sRIZroposalNameIn, std::string strURLIn, int nPaymentCount, CScript addressIn, CAmount nAmountIn, int nBlockStartIn, uint256 nFeeTXHashIn);
    ...
    void swap(CBudgetProposalBroadcast& first, CBudgetProposalBroadcast& second) // nothrow
    {
        ...
        // by swapping the members of two classes,
        // the two classes are effectively swapped
        swap(first.sRIZroposalName, second.sRIZroposalName);
        swap(first.nBlockStart, second.nBlockStart);
