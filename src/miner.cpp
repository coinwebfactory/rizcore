...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("RizMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("RizMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("RizMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("riz-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running RizMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
