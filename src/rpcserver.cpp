...
//! These are created by StarRIZCThreads, destroyed in StopRPCThreads
    ...
    bool fAllowNull)
{
    ...
    BOOST_FOREACH (Value_type t, typesExpected) {
        ...
        if (!((v.type() == t) || (fAllowNull && (v.type() == null_type)))) {
            string err = sRIZrintf("Expected type %s, got %s",
    ...
    bool fAllowNull)
{
    BOOST_FOREACH (const PAIRTYPE(string, Value_type) & t, typesExpected) {
        ...
        if (!fAllowNull && v.type() == null_type)
            throw JSONRPCError(RPC_TYPE_ERROR, sRIZrintf("Missing %s", t.first));

        if (!((v.type() == t.second) || (fAllowNull && (v.type() == null_type)))) {
            string err = sRIZrintf("Expected type %s for %s, got %s",
...
string CRPCTable::help(string strCommand) const
{
    ...
    if (strRet == "")
        strRet = sRIZrintf("help: unknown command: %s\n", strCommand);
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* Riz features */
        {"riz", "masternode", &masternode, true, true, false},
        {"riz", "masternodelist", &masternodelist, true, true, false},
        {"riz", "mnbudget", &mnbudget, true, true, false},
        {"riz", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"riz", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"riz", "mnsync", &mnsync, true, true, false},
        {"riz", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"riz", "obfuscation", &obfuscation, false, false, true}, /* not threadSafe because of SendMoney */
...
void StarRIZCThreads()
{
        ...
        BOOST_FOREACH (string strAllow, vAllow) {
            ...
            if (!subnet.IsValid()) {
                uiInterface.ThreadSafeMessageBox(
                    sRIZrintf("Invalid -rpcallowip subnet specification: %s. Valid are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24).", strAllow),
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(sRIZrintf(
                                             _("To use rizd, or the -server option to riz-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
    ...
    } else if (mapArgs.count("-rpcbind")) // Specific bind address
    {
        BOOST_FOREACH (const std::string& addr, mapMultiArgs["-rpcbind"]) {
            try {
                ...
                uiInterface.ThreadSafeMessageBox(
                    sRIZrintf(_("Could not parse -rpcbind value %s as network address"), addr),
                    "", CClientUIInterface::MSG_ERROR);
                StartShutdown();
                return;
            }
    ...
    BOOST_FOREACH (const ip::tcp::endpoint& endpoint, vEndpoints) {
        try {
            ...
            strerr = sRIZrintf(_("An error occurred while setting up the RPC address %s port %u for listening: %s"), straddress, endpoint.port(), e.what());
        }
    }
...
void SeRIZCWarmupStatus(const std::string& newStatus)
{
    LOCK(cs_rpcWarmup);
    rpcWarmupStatus = newStatus;
}

void SeRIZCWarmupFinished()
{
    LOCK(cs_rpcWarmup);
    assert(fRPCInWarmup);
    fRPCInWarmup = false;
...
{
    return "> riz-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:7198/\n";
}
