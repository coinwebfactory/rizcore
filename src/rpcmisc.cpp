...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized riz balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in riz/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in riz/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"rizaddress\"\n"
            "\nReturn information about the given riz address.\n"
            "\nArguments:\n"
            "1. \"rizaddress\"     (string, required) The riz address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"rizaddress\", (string) The riz address validated\n"
...
CScript _createmultisig_redeemScript(const Array& params)
{
    ...
    if ((int)keys.size() < nRequired)
        throw runtime_error(
            sRIZrintf("not enough keys supplied "
...
#ifdef ENABLE_WALLET
        // Case 1: Riz address and we have full public key:
        CBitcoinAddress address(ks);
        if (pwalletMain && address.IsValid()) {
            CKeyID keyID;
            if (!address.GetKeyID(keyID))
                throw runtime_error(
                    sRIZrintf("%s does not refer to a key", ks));
            CPubKey vchPubKey;
            if (!pwalletMain->GetPubKey(keyID, vchPubKey))
                throw runtime_error(
                    sRIZrintf("no full public key for address %s", ks));
    ...
    if (result.size() > MAX_SCRIPT_ELEMENT_SIZE)
        throw runtime_error(
            sRIZrintf("redeemScript exceeds size limit: %d > %d", result.size(), MAX_SCRIPT_ELEMENT_SIZE));
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are riz addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) riz address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"rizaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"rizaddress\"  (string, required) The riz address to use for the signature.\n"
