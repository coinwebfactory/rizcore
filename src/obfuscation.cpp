...
std::string CObfuscationPool::GetStatus()
{
    ...
    case POOL_STATUS_ACCEPTING_ENTRIES:
        if (entriesCount == 0) {
            ...
            if (showingObfuScationMessage % 70 <= 40)
                return sRIZrintf(_("Submitted following entries to masternode: %u / %d"), entriesCount, GetMaxPoolTransactions());
                ...
                suffix = "...";
            return sRIZrintf(_("Submitted to masternode, waiting for more entries ( %u / %d ) %s"), entriesCount, GetMaxPoolTransactions(), suffix);
        }
    case POOL_STATUS_SIGNING:
        if (showingObfuScationMessage % 70 <= 40)
            ...
            suffix = "...";
        return sRIZrintf(_("Found enough users, signing ( waiting %s )"), suffix);
    ...
    case POOL_STATUS_QUEUE:
        if (showingObfuScationMessage % 70 <= 30)
            ...
            suffix = "...";
        return sRIZrintf(_("Submitted to masternode, waiting in queue %s"), suffix);
        ;
    default:
        return sRIZrintf(_("Unknown state: id = %u"), state);
    }
}
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Riz and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 RIZ per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeRizAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100RIZ+1 ( bit on if present )
    // bit 1 - 10RIZ+1
    // bit 2 - 1RIZ+1
    // bit 3 - .1RIZ+1
    // bit 3 - non-denom
