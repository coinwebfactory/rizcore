...
void ComputePreFactor(std::string sRIZassphrase, std::string strSalt, uint256& prefactor)
{
    ...
    scrypt_hash(sRIZassphrase.c_str(), sRIZassphrase.size(), BEGIN(s), strSalt.size() / 2, BEGIN(prefactor), 16384, 8, 8, 32);
}
...
std::string BIP38_Encrypt(std::string strAddress, std::string sRIZassphrase, uint256 privKey)
{
    string strAddressHash = AddressToBip38Hash(strAddress);

    uint512 hashed;
    uint64_t salt = uint256(ReverseEndianString(strAddressHash)).Get64();
    scrypt_hash(sRIZassphrase.c_str(), sRIZassphrase.size(), BEGIN(salt), strAddressHash.size() / 2, BEGIN(hashed), 16384, 8, 8, 64);
...
bool BIP38_Decrypt(std::string sRIZassphrase, std::string strEncryptedKey, uint256& privKey, bool& fCompressed)
{
    ...
    uint256 prefactor;
    ComputePreFactor(sRIZassphrase, prefactorSalt, prefactor);
