...
std::string CNetAddr::ToStringIP() const
{
    ...
    if (IsIPv4())
        return sRIZrintf("%u.%u.%u.%u", GetByte(3), GetByte(2), GetByte(1), GetByte(0));
    else
        return sRIZrintf("%x:%x:%x:%x:%x:%x:%x:%x",
...
std::string CSubNet::ToString() const
{
    ...
    if (network.IsIPv4())
        strNetmask = sRIZrintf("%u.%u.%u.%u", netmask[12], netmask[13], netmask[14], netmask[15]);
    else
        strNetmask = sRIZrintf("%x:%x:%x:%x:%x:%x:%x:%x",
...
std::string NetworkErrorString(int err)
{
    ...
    if (FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK,
            ...
            buf, sizeof(buf), NULL)) {
        return sRIZrintf("%s (%d)", buf, err);
    } else {
        return sRIZrintf("Unknown error (%d)", err);
    }
}
#else
std::string NetworkErrorString(int err)
