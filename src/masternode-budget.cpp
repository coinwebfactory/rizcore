...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf)
{
    ...
    if (!GetTransaction(nTxCollateralHash, txCollateral, nBlockHash, true)) {
        strError = sRIZrintf("Can't find collateral tx %s", txCollateral.ToString());
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        if (!o.scriptPubKey.IsNormalPaymentScript() && !o.scriptPubKey.IsUnspendable()) {
            strError = sRIZrintf("Invalid Script %s", txCollateral.ToString());
            LogPrintf("CBudgetProposalBroadcast::IsBudgetCollateralValid - %s\n", strError);
            return false;
        }
    ...
    if (!foundOpReturn) {
        strError = sRIZrintf("Couldn't find opReturn %s in %s", nExpectedHash.ToString(), txCollateral.ToString());
        LogPrintf("CBudgetProposalBroadcast::IsBudgetCollateralValid - %s\n", strError);
        return false;
    }
    ...
    */
    ...
    if (conf >= BUDGET_FEE_CONFIRMATIONS) {
        ...
        strError = sRIZrintf("Collateral requires at least %d confirmations - %d confirmations", BUDGET_FEE_CONFIRMATIONS, conf);
...
CBudgetProposal* CBudgetManager::FindProposal(const std::string& sRIZroposalName)
{
    ...
    while (it != mapProposals.end()) {
        if ((*it).second.sRIZroposalName == sRIZroposalName && (*it).second.GetYeas() > nYesCount) {
...
CAmount CBudgetManager::GetTotalBudget(int nHeight)
{
    ...
    if (nHeight <= 200 && nHeight > 0) {
        nSubsidy = 0 * COIN;
    } else if (nHeight <= 3500 && nHeight >= 0) {
        nSubsidy = 3 * COIN;
...
CBudgetProposal::CBudgetProposal(std::string sRIZroposalNameIn, std::string strURLIn, int nBlockStartIn, int nBlockEndIn, CScript addressIn, CAmount nAmountIn, uint256 nFeeTXHashIn)
{
    sRIZroposalName = sRIZroposalNameIn;
    strURL = strURLIn;
...
{
    sRIZroposalName = other.sRIZroposalName;
    strURL = other.strURL;
...
bool CBudgetProposal::AddOrUpdateVote(CBudgetVote& vote, std::string& strError)
{
    ...
    if (vote.nTime > GetTime() + (60 * 60)) {
        strError = sRIZrintf("new vote is too far ahead of current time - %s - nTime %lli - Max Time %lli\n", vote.GetHash().ToString(), vote.nTime, GetTime() + (60 * 60));
        LogPrint("mnbudget", "CBudgetProposal::AddOrUpdateVote - %s\n", strError);
        return false;
    }
...
CBudgetProposalBroadcast::CBudgetProposalBroadcast(std::string sRIZroposalNameIn, std::string strURLIn, int nPaymentCount, CScript addressIn, CAmount nAmountIn, int nBlockStartIn, uint256 nFeeTXHashIn)
{
    sRIZroposalName = sRIZroposalNameIn;
    strURL = strURLIn;
