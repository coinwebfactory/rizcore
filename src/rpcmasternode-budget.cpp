...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  vote-many          - Vote on a Riz initiative\n"
            "  vote-alias         - Vote on a Riz initiative\n"
            "  vote               - Vote on a Riz initiative/budget\n"
    ...
    if (strCommand == "prepare") {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'mnbudget prepare proposal-name url payment_count block_start riz_address monthly_payment_riz'");

        std::string sRIZroposalName = params[1].get_str();
        if (sRIZroposalName.size() > 20)
        ...
        if (nBlockStart % GetBudgetPaymentCycleBlocks() != 0) {
            int nNext = pindexPrev->nHeight - pindexPrev->nHeight % GetBudgetPaymentCycleBlocks() + GetBudgetPaymentCycleBlocks();
            return sRIZrintf("Invalid block start - must be a budget cycle block. Next valid block: %d", nNext);
        }
        ...
        CBudgetProposalBroadcast budgetProposalBroadcast(sRIZroposalName, strURL, nPaymentCount, scriptPubKey, nAmount, nBlockStart, 0);
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start riz_address monthly_payment_riz fee_tx'");
        ...
        CBudgetProposalBroadcast budgetProposalBroadcast(sRIZroposalName, strURL, nPaymentCount, scriptPubKey, nAmount, nBlockStart, hash);
    ...
    if (strCommand == "vote-many") {
        ...
        Object returnObj;
        returnObj.push_back(Pair("overall", sRIZrintf("Voted successfully %d time(s) and failed %d time(s).", success, failed)));
    ...
    if (strCommand == "getinfo") {
        ...
        CBudgetProposal* pbudgetProposal = budget.FindProposal(sRIZroposalName);
