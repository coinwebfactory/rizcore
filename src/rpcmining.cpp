...
// Allocated in IniRIZCMining, free'd in ShutdownRPCMining
...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or riz.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Riz is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Riz is downloading blocks...");
...
// Release the wallet and main lock while waiting
    ...
    result.push_back(Pair("bits", sRIZrintf("%08x", pblock->nBits)));
