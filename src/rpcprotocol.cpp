...
string HTTPPost(const string& strMsg, const map<string, string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: riz-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
string HTTPError(int nStatus, bool keepalive, bool headersOnly)
{
    if (nStatus == HTTP_UNAUTHORIZED)
        return sRIZrintf("HTTP/1.0 401 Authorization Required\r\n"
                         "Date: %s\r\n"
                         "Server: riz-json-rpc/%s\r\n"
...
bool ReadHTTPRequestLine(std::basic_istream<char>& stream, int& proto, string& http_method, string& http_uri)
{
    ...
    proto = 0;
    const char* ver = strstr(sRIZroto.c_str(), "HTTP/1.");
    if (ver != NULL)
        proto = atoi(ver + 7);
...
/**
 * JSON-RPC protocol.  Riz speaks version 1.0 for maximum compatibility,
