...
//
// Bootup the Masternode, look for a 1000 Riz input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN) {
            if (service.GetPort() != 7199) {
                notCapableReason = sRIZrintf("Invalid port: %u - only 7199 is supported on mainnet.", service.GetPort());
                LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
                return;
            }
        } else if (service.GetPort() == 7199) {
            notCapableReason = sRIZrintf("Invalid port: %u - 7199 is only supported on mainnet.", service.GetPort());
            LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
            return;
        }
        ...
        if (GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            if (GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
                ...
                notCapableReason = sRIZrintf("%s - %d confirmations", GetStatus(), GetInputAge(vin));
                LogPrintf("CActiveMasternode::ManageStatus() - %s\n", notCapableReason);
                return;
            }
...
std::string CActiveMasternode::GetStatus()
{
    ...
    case ACTIVE_MASTERNODE_INPUT_TOO_NEW:
        return sRIZrintf("Masternode input must have at least %d confirmations", MASTERNODE_MIN_CONFIRMATIONS);
...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage)
{
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, keyMasternode, pubKeyMasternode)) {
        errorMessage = sRIZrintf("Error upon calling SetKey: %s\n", errorMessage);
        return false;
    }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    if (!obfuScationSigner.SetKey(strKeyMasternode, errorMessage, keyMasternode, pubKeyMasternode)) {
        errorMessage = sRIZrintf("Can't find keys for masternode %s - %s", strService, errorMessage);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }

    if (!GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress, strTxHash, strOutputIndex)) {
        errorMessage = sRIZrintf("Could not allocate vin %s:%s for masternode %s", strTxHash, strOutputIndex, strService);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN) {
        if (service.GetPort() != 7199) {
            errorMessage = sRIZrintf("Invalid port %u for masternode %s - only 7199 is supported on mainnet.", service.GetPort(), strService);
            LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
            return false;
        }
    } else if (service.GetPort() == 7199) {
        errorMessage = sRIZrintf("Invalid port %u for masternode %s - 7199 is only supported on mainnet.", service.GetPort(), strService);
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
...
bool CActiveMasternode::Register(CTxIn vin, CService service, CKey keyCollateralAddress, CPubKey pubKeyCollateralAddress, CKey keyMasternode, CPubKey pubKeyMasternode, std::string& errorMessage)
{
    ...
    CMasternodePing mnp(vin);
    if (!mnp.Sign(keyMasternode, pubKeyMasternode)) {
        errorMessage = sRIZrintf("Failed to sign ping, vin: %s", vin.ToString());
        LogPrintf("CActiveMasternode::Register() -  %s\n", errorMessage);
        return false;
    }
    mnodeman.mapSeenMasternodePing.insert(make_pair(mnp.GetHash(), mnp));
    ...
    if (!mnb.Sign(keyCollateralAddress)) {
        errorMessage = sRIZrintf("Failed to sign broadcast, vin: %s", vin.ToString());
        LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
        return false;
    }
