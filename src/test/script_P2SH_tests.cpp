...
BOOST_AUTO_TEST_CASE(sign)
{
...
#ifdef ENABLE_WALLET
        BOOST_CHECK_MESSAGE(IsMine(keystore, txFrom.vout[i].scriptPubKey), sRIZrintf("IsMine %d", i));
#endif
    }
    for (int i = 0; i < 8; i++)
    {
    ...
    for (int i = 0; i < 8; i++)
        for (int j = 0; j < 8; j++)
        {
            ...
            if (i == j)
                BOOST_CHECK_MESSAGE(sigOK, sRIZrintf("VerifySignature %d %d", i, j));
            else
                BOOST_CHECK_MESSAGE(!sigOK, sRIZrintf("VerifySignature %d %d", i, j));
...
BOOST_AUTO_TEST_CASE(set)
{
    ...
    {
        BOOST_CHECK_MESSAGE(SignSignature(keystore, txFrom, txTo[i], 0), sRIZrintf("SignSignature %d", i));
        BOOST_CHECK_MESSAGE(IsStandardTx(txTo[i], reason), sRIZrintf("txTo[%d].IsStandard", i));
    }
}
