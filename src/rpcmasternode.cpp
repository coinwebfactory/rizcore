...
Value obfuscation(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <rizaddress> <amount>\n"
            "rizaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <rizaddress> <amount>\n"
            "rizaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in riz.conf\n"
    ...
    if (strCommand == "count") {
        ...
        if (params.size() == 2) {
            ...
            if (params[1] == "all") return sRIZrintf("Total: %d (OBF Compatible: %d / Enabled: %d / Qualify: %d)",
                mnodeman.size(),
    ...
    if (strCommand == "start-many" || strCommand == "start-all" || strCommand == "start-missing" || strCommand == "start-disabled") {
        ...
        Object returnObj;
        returnObj.push_back(Pair("overall", sRIZrintf("Successfully started %d masternodes, failed to start %d, total %d", successful, failed, successful + failed)));
    ...
    if (strCommand == "outputs") {
        ...
        BOOST_FOREACH (COutput& out, possibleCoins) {
            obj.push_back(Pair(out.tx->GetHash().ToString(), sRIZrintf("%d", out.i)));
        }
    ...
    if (strCommand == "winners") {
        ...
        for (int nHeight = chainActive.Tip()->nHeight - nLast; nHeight < chainActive.Tip()->nHeight + 20; nHeight++) {
            obj.push_back(Pair(sRIZrintf("%d", nHeight), GetRequiredPaymentsString(nHeight)));
        }
    ...
    */
    if (strCommand == "calcscore") {
        ...
        for (int nHeight = chainActive.Tip()->nHeight - nLast; nHeight < chainActive.Tip()->nHeight + 20; nHeight++) {
            ...
            if (pBestMasternode)
                obj.push_back(Pair(sRIZrintf("%d", nHeight), pBestMasternode->vin.prevout.ToStringShort().c_str()));
        }
    ...
    if (strMode == "rank") {
        ...
        BOOST_FOREACH (CMasternode& mn, vMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                if (strFilter != "" && strFilter != sRIZrintf("%d", mn.protocolVersion) &&
                    strVin.find(strFilter) == string::npos) continue;
