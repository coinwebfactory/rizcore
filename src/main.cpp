...
#if defined(NDEBUG)
#error "Riz cannot be compiled without assertions."
#endif
            ...
            for (unsigned int i = 0; i < tx.vin.size(); i++) {
                ...
                if (pvChecks) {
                    ...
                    if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
                        ...
                        if (check())
                            return state.Invalid(false, REJECT_NONSTANDARD, sRIZrintf("non-mandatory-script-verify-flag (%s)", ScriptErrorString(check.GetScriptError())));
                    }
                    ...
                    return state.DoS(100, false, REJECT_INVALID, sRIZrintf("mandatory-script-verify-flag-failed (%s)", ScriptErrorString(check.GetScriptError())));
                }
            }
        }
    }
...
{
    return GetDataDir() / "blocks" / sRIZrintf("%s%05u.dat", prefix, pos.nFile);
}
...
bool static LoadBlockIndexDB()
{
    ...
    if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
        ...
        if (!ReadBlockFromDisk(lastMetaBlock, pindexLastMeta)) {
            isFixed = false;
            strError = sRIZrintf("failed to read block %d from disk", pindexLastMeta->nHeight);
        }
        ...
        if (!ReadBlockFromDisk(lastBlock, pindexLast)) {
            isFixed = false;
            strError = sRIZrintf("failed to read block %d from disk", pindexLast->nHeight);
        }
    ...
    if (strCommand == "version") {
        ...
        if (pfrom->nVersion < ActiveProtocol()) {
            ...
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
                sRIZrintf("Version must be %d or greater", ActiveProtocol()));
...
{
    return sRIZrintf("CBlockFileInfo(blocks=%u, size=%u, heights=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst, nHeightLast, DateTimeStrFormat("%Y-%m-%d", nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
}
