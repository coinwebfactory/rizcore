...
void MasternodeList::StartAll(std::string strCommand)
{
    ...
    returnObj = sRIZrintf("Successfully started %d masternodes, failed to start %d, total %d", nCountSuccessful, nCountFailed, nCountFailed + nCountSuccessful);
    if (nCountFailed > 0) {
...
void MasternodeList::updateNodeList()
{
    ...
    if (fFilterUpdated) ui->countLabel->setText(QString::fromStdString(sRIZrintf("Please wait... %d", nSecondsToWait)));
...
void MasternodeList::on_filterLineEdit_textChanged(const QString& strFilterIn)
{
    ...
    nTimeFilterUpdated = GetTime();
    fFilterUpdated = true;
    ui->countLabel->setText(QString::fromStdString(sRIZrintf("Please wait... %d", MASTERNODELIST_FILTER_COOLDOWN_SECONDS)));
}
