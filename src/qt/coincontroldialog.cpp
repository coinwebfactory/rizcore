...
QString CoinControlDialog::sRIZad(QString s, int nPadLength, QString sPadding)
{
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::RIZ;
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        BOOST_FOREACH (const COutput& out, coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show Riz address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            itemOutput->setText(COLUMN_AMOUNT_INT64, sRIZad(QString::number(out.tx->vout[out.i].nValue), 15, " ")); // padding so that sorting works correctly

            // date
            itemOutput->setText(COLUMN_DATE, GUIUtil::dateTimeStr(out.tx->GetTxTime()));
            itemOutput->setToolTip(COLUMN_DATE, GUIUtil::dateTimeStr(out.tx->GetTxTime()));
            itemOutput->setText(COLUMN_DATE_INT64, sRIZad(QString::number(out.tx->GetTxTime()), 20, " "));
            ...
            if (rounds >= 0)
                itemOutput->setText(COLUMN_OBFUSCATION_ROUNDS, sRIZad(QString::number(rounds), 11, " "));
            else
                itemOutput->setText(COLUMN_OBFUSCATION_ROUNDS, sRIZad(QString(tr("n/a")), 11, " "));


            // confirmations
            itemOutput->setText(COLUMN_CONFIRMATIONS, sRIZad(QString::number(out.nDepth), 8, " "));

            // priority
            double dPriority = ((double)out.tx->vout[out.i].nValue / (nInputSize + 78)) * (out.nDepth + 1); // 78 = 2 * 34 + 10
            itemOutput->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPriority, mempoolEstimatePriority));
            itemOutput->setText(COLUMN_PRIORITY_INT64, sRIZad(QString::number((int64_t)dPriority), 20, " "));
        ...
        if (treeMode) {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT_INT64, sRIZad(QString::number(nSum), 15, " "));
            itemWalletAddress->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPrioritySum, mempoolEstimatePriority));
            itemWalletAddress->setText(COLUMN_PRIORITY_INT64, sRIZad(QString::number((int64_t)dPrioritySum), 20, " "));
        }
    }
