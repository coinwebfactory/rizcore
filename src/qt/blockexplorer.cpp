...
static std::string ValueToString(int64_t nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::RIZ, nValue);
    ...
    std::string BlockContentCells[] =
        {
            ...
            _("Difficulty"), sRIZrintf("%.4f", GetDifficulty(pBlock)),
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (riz.conf).");
            QMessageBox::warning(this, "Riz Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
