...
bool isValidPassphrase(QString sRIZassphrase, QString& strInvalid)
{
    for (int i = 0; i < sRIZassphrase.size(); i++) {
        if (!validChar.contains(sRIZassphrase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(sRIZassphrase[i]))
                continue;

            strInvalid = sRIZassphrase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qsRIZassphrase = ui->passphraseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassphrase(qsRIZassphrase, strInvalid)) {
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qsRIZassphrase.toStdString(), key.GetPrivKey_256());
...
{
    string sRIZassphrase = ui->passphraseIn_DEC->text().toStdString();
