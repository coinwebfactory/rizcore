...
Value listunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of riz addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) riz address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the riz address\n"
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH (Value& input, inputs) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Riz address: ") + input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the riz address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) riz address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value sendrawtransaction(const Array& params, bool fHelp)
{
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        if (!AcceptToMemoryPool(mempool, state, tx, false, NULL, !fOverrideFees)) {
            if (state.IsInvalid())
                throw JSONRPCError(RPC_TRANSACTION_REJECTED, sRIZrintf("%i: %s", state.GetRejectCode(), state.GetRejectReason()));
            else
